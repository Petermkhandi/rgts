...
$pending_count = $db->query("SELECT COUNT(DISTINCT graduate_id) FROM verification_logs WHERE verification_status != 'verified' OR graduate_id NOT IN (SELECT graduate_id FROM verification_logs)")->fetchColumn();
...
$stmt = $db->query("
    SELECT g.course, COUNT(g.id) as total,
           ...
           SUM(CASE WHEN ed.employment_status = 'unemployed' THEN 1 ELSE 0 END) as unemployed,
           SUM(CASE WHEN ed.employment_status = 'further_studies' THEN 1 ELSE 0 END) as studies,
           SUM(CASE WHEN ed.employment_status = 'seeking' THEN 1 ELSE 0 END) as seeking
    FROM graduates g
...
// Salary distribution
$stmt = $db->query("SELECT salary_range, COUNT(*) as count FROM employment_details WHERE salary_range IS NOT NULL GROUP BY salary_range ORDER BY salary_range");
...
$org_stats = $stmt->fetchAll();

// Location distribution
$stmt = $db->query("SELECT location, COUNT(*) as count FROM employment_details WHERE location IS NOT NULL GROUP BY location ORDER BY count DESC LIMIT 10");
    ...
    <div class="col-md-3">
        <div class="card stat-card bg-warning text-dark">
            <div class="card-body text-center">
                ...
                <p class="mb-0">Further Studies</p>
            </div>
        </div>
    </div>
</div>
